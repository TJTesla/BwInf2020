...
def printFinished(searchedItem):
    ...
    print('')
    for searchedWord in searchedItem:
        print(searchedWord.origIndex, end='  ')
    print('')


def findSolutionWithWords(searched, given, cat):
    for jIter in range(len(searched[cat])):
        counter = 0
        hiddenIndex = int
        for iterate in range(len(given[cat])-1, -1, -1):
            if wouldFit(searched[cat][jIter], given[cat][iterate]) and searched[cat][jIter].completedWord == '':
                counter += 1
                hiddenIndex = iterate
        if counter == 1:
            searched[cat][jIter].completedWord = given[cat][hiddenIndex]
            del given[cat][hiddenIndex]
...
def recursiveSolution(searched, given, cat):
    ...
    if len(given[cat]) != 0:  # See notes for logical reference
        findSolutionWithWords(searched, given, cat)
